<script lang="ts">
	...
	// Pre-fill selected items from existing data
	$effect(() => {
		if (data.preselectedEquipmentIds && Object.keys(selectedItems).length === 0) {
			const initial: Record<string, any> = {};
			// We only have equipment IDs, we need to map them back to groups when data loads
			// But initially, we can just store the manual IDs.
			// The $effect below handles merging loaded group data with these pre-selections.
		}
	});
	...
	$effect(() => {
		...
		getAvailableEquipment({
			...
			q: queryStr,
			page: pageNum,
			preselectedEquipmentIds: data.preselectedEquipmentIds || []
		})
			.then((res) => {
			...
			untrack(() => {
				...
				groupedEquipment.forEach((group: any) => {
					if (updated[group.id] === undefined) {
						...
						// If preselected, the group might have multiple preselectedIds
						const preselectedIds = group.preselectedIds || [];

						updated[group.id] = {
							selected: isPreselected,
							qty: isPreselected ? preselectedIds.length : 0,
							manualIds: preselectedIds,
							itemId: group.itemId,
...
<svelte:head>
	<title>Edit Peminjaman | {data.targetOrg?.name || 'MINMAT'}</title>
</svelte:head>
		...
		<div class="flex items-center justify-between">
			<div>
				...
				<p class="mt-1 text-sm text-muted-foreground">
					Perbarui informasi dan daftar alat peminjaman.
				</p>
			</div>
		...
		<form method="POST" enctype="multipart/form-data" use:superEnhance class="space-y-6">
			...
			<div class="rounded-lg border bg-card p-6 shadow-sm">
				...
				<div class="grid gap-6 md:grid-cols-2">
					<div class="space-y-2">
						<Label for="unit">Unit/Satuan Peminjam <span class="text-red-500"> * </span></Label>
						<Input
							id="unit"
							name="unit"
					...
					<div class="col-span-2 space-y-2">
						<Label for="attachment" class="flex items-center gap-2">
							Dokumen Pendukung (Surat Perintah/Permohonan) Baru
						</Label>
						<Input
							id="attachment"
							name="attachment"
						...
						<p class="text-[10px] text-muted-foreground uppercase">
							Kosongkan jika tidak ingin mengubah dokumen sebelumnya. Format yang diterima: .PDF,
							.DOCX (Maks 5MB)
						</p>
							...
							<div class="mt-2">
								<p class="text-xs text-muted-foreground">
									Dokumen saat ini: <a
										...
										>{data.lending.attachmentName || 'Lampiran'}</a
									>
								</p>
							</div>
						{/if}
			...
			{#each Object.entries(selectedItems) as [groupId, item]}
				{#if item.selected}
					{#if item.manualIds && item.manualIds.length > 0}
						{#each item.manualIds as manualId (manualId)}
	...
	{:else}
		<div class="rounded-lg border-2 border-dashed bg-muted/50 p-20 text-center">
			<p class="text-xl font-medium text-muted-foreground">
				Tidak dapat memuat organisasi peminjaman.
			</p>
		</div>
	{/if}
